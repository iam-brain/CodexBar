...
struct CLISnapshotTests {
    ...
    @Test
    func rendersSparkSectionWhenWindowsExistWithoutProPlanMetadata() {
        let now = Date()
        let snap = UsageSnapshot(
            primary: .init(
                usedPercent: 22,
        ...
        let output = CLIRenderer.renderText(
            ...
            snapshot: snap,
            credits: nil,
            accountInfo: AccountInfo(email: "codex@example.com", plan: "Plus"),
            context: RenderContext(
                header: "Codex 0.0.0 (codex-cli)",
                status: nil,
                useColor: false,
        ...
        #expect(output.contains("GPT-5.3-Codex-Spark"))
        #expect(output.contains("Plan: Plus"))
    }
